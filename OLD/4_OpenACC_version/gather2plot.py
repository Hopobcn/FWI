...
def main():
    ...
    for line in open(fname, 'r'):
        if ('STATS' in line) and ('GLOBAL' in line):
            ...
            sys.stdout.write(jstr+' '+istest+' '+sline[6]+' '+sline[9]+'\n')
            
            j += 1


    with open('fwi.openacc.csv', 'w') as csvfile:
        writer  = csv.writer(csvfile)
        #writer.writerow(['freq','metric'])
        ...
        for fid in range(0, nfreqs):
            ...
            #print >> sys.stderr, 'Freq: ', freqs[fid].rstrip(), 'TIME     - Mean:', '  {0:.8f}'.format(np.mean(time)), 'Median:', '  {0:.8f}'.format(np.median(time)), 'Max:', '  {0:.8f}'.format(np.amax(time)), 'Min:', '  {0:.8f}'.format(np.amin(time)), 'STD:', np.std(time)
