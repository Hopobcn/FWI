...
As an extra, you should move the data initialization to the gpu by puting a `#pragma acc kernels` before the loop in function `set_array_to_constant` of `src/fwi_kernels.c`. Example:
...
[100%] Built target irun
```
This time we obtained a slight improvement.
...
For demonstration purposes we include a preliminar version of the new OmpSs/OpenACC interoperability. Be aware that it's still in development and problems may occur.
...
```
At the moment we only support the `kernels` and `loop` directive with clauses `deviceptr`. Any other combination is not supported.
